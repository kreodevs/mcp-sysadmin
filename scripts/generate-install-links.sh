#!/usr/bin/env bash
# Genera enlaces/botones de instalación MCP (GitHub Packages vía npx, o modo local).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
NAME="${MCP_SERVER_NAME:-mcp-sysadmin}"
PACKAGE="${MCP_PACKAGE:-@kreodevs/mcp-sysadmin}"
REGISTRY="${MCP_REGISTRY:-https://npm.pkg.github.com}"
MODE="${INSTALL_MODE:-package}"
CONFIRM_TOKEN="${SYSADMIN_CONFIRM_TOKEN:-CHANGE_ME_openssl_rand_hex_32}"
INVENTORY="${SYSADMIN_INVENTORY_PATH:-/path/to/inventory.json}"
...
if [[ "$MODE" == "local" ]]; then
  ...
  if [[ -f "$ROOT/config/inventory.json" ]]; then
    INVENTORY="$ROOT/config/inventory.json"
  elif [[ "$INVENTORY" == "/path/to/inventory.json" ]]; then
...
const c = {
  command: 'npx',
  args: ['-y', '--registry=' + process.argv[1], process.argv[2]],
  env: {
...
process.stdout.write(JSON.stringify(c));
" "$REGISTRY" "$PACKAGE" "$INVENTORY" "$CONFIRM_TOKEN")
fi

VSCODE_CONFIG=$(node -e "
const c = JSON.parse(process.argv[2]);
console.log(JSON.stringify({ name: process.argv[1], ...c }));
" "$NAME" "$CURSOR_CONFIG")
...
npx -y --registry=$REGISTRY $PACKAGE
...
$(node -e "
const cfg = { mcpServers: { [process.argv[1]]: JSON.parse(process.argv[2]) } };
...
const cfg = {
  ...
  mcp: {
    [process.argv[2]]: {
      type: "local",
      command: base.command === "npx" ? [base.command, ...base.args] : [base.command],
      enabled: true,
